<template>
  <nav :class="[
    'fixed bottom-0 left-0 right-0 z-40 lg:hidden border-t shadow-[0_-2px_12px_rgba(0,0,0,0.1)] nav-safe transition-colors duration-200',
    theme.isDark
    ...
    <div class="flex overflow-x-auto nav-scroll">
      ...
      >
        <div
          ...
          :class="['absolute top-0 left-1/2 -translate-x-1/2 w-8 h-[3px] rounded-full', theme.isDark ? 'bg-[#4ade80]' : 'bg-[#006d35]']"
        />
        <component
          :is="item.icon"
