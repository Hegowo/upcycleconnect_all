...
func Setup(db *gorm.DB, cfg *config.Config) *gin.Engine {
	...
	userAPI := r.Group("/api/v1")
	{
		...
		userProtected.Use(middleware.AuthMiddleware(db, cfg))
		{
			...
			userProtected.POST("/projects/:id/steps/:step_id/images", projectHandler.AddStepImage)
			userProtected.DELETE("/projects/:id/steps/:step_id/images/:image_id", projectHandler.DeleteStepImage)
			...
			userProtected.GET("/provider/reservations", userProviderHandler.ListReservations)
	...
	api := r.Group("/api/admin/v1")
	{
		...
		protected.Use(middleware.IsAdmin())
		{
			...
			protected.DELETE("/subscription-plans/:key", subscriptionHandler.AdminDeletePlan)
